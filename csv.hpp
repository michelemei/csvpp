...
namespace michelemei {
    ...
    /// Fields container
    ///
    /// @brief It's just a `std::vector<string>` inheritance decorated with
    /// some utilities for string management, i.e. lower/upper-case conversion,
    //  characters eraser/replacer, suffix and prefix
    template<class _Elem, class _Traits>
    struct csv_line : public std::vector<std::basic_string<_Elem, _Traits>>
    {
        using me = csv_line<_Elem, _Traits>;
        me&& lowercase() &&
        {
            for (auto& f : *this)
        ...
        me&& erase(std::initializer_list<_Elem>&& chars) &&
        {
            for (auto& f : *this)
            {
                for (auto& c : chars)
        ...
        me&& replace(std::initializer_list<std::pair<_Elem, _Elem>>&& pairs) &&
        {
            for (auto& f : *this)
            {
                for (auto& p : pairs)
                {
                    std::replace(f.begin(), f.end(), p.first, p.second);
                }
            }
            return std::move(*this);
        }
        me&& suffix(const std::basic_string<_Elem>& _suffix) &&
        {
            for (auto& f : *this)
                f += _suffix;
        ...
        me&& prefix(const std::basic_string<_Elem>& _prefix) &&
        {
            for (auto& f : *this)
                f = _prefix + f;
...
#define ENABLE_COMPILETIME_STRING_HASH
#if defined(ENABLE_COMPILETIME_STRING_HASH)
...
// TODO: try to write a fold expression alternative to recursive Crc32Impl
